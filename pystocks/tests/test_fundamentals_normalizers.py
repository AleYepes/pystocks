...
class FundamentalsNormalizersTests(unittest.TestCase):
    def test_ratios_factor_feature_extraction(self):
        payload = {
            "ratios": [
                {
                    "name": "Price/Sales",
        ...
        self.assertIn("fundamentals_price_sales", names)
        self.assertIn("fundamentals_price_sales_vs", names)
        self.assertIn("fundamentals_price_sales_percentile", names)

    def test_holdings_weighted_features(self):
        payload = {
            "allocation_self": [
        ...
        self.assertAlmostEqual(by_name["holding_types_equity"], 0.995, places=6)
        self.assertAlmostEqual(by_name["industries_technology"], 0.448, places=6)
        self.assertAlmostEqual(by_name["currencies_usd"], 0.982, places=6)
        self.assertAlmostEqual(by_name["countries_united_states"], 0.75, places=6)
        self.assertAlmostEqual(by_name["holding_top_10_weight"], 0.3839, places=6)

    def test_morningstar_ordinal_mapping(self):
        payload = {
            "summary": [
        ...
        by_name = {r["feature_name"]: r["feature_value"] for r in rows}
        self.assertEqual(by_name["morningstar_medalist_rating"], 3.0)
        self.assertEqual(by_name["morningstar_sustainability_rating"], 4.0)
        self.assertEqual(by_name["morningstar_morningstar_rating"], 2.0)

    def test_ownership_trade_log_drops_no_change(self):
        payload = {
            "trade_log": [
    ...
    def test_dividends_events_ignore_price_series(self):
        payload = {
            "history": {
                "series": [
                    {
